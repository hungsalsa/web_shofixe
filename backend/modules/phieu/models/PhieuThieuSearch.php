<?php

namespace backend\modules\phieu\models;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use backend\modules\phieu\models\PhieuThieu;

/**
 * PhieuThieuSearch represents the model behind the search form of `backend\modules\phieu\models\PhieuThieu`.
 */
class PhieuThieuSearch extends PhieuThieu
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['id', 'cuahang_id','created_at', 'updated_at', 'user_add'], 'integer'],
            [['ngay_giao', 'note', 'so_phieu',  'status'], 'safe'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = PhieuThieu::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id' => $this->id,
            'ngay_giao' => $this->ngay_giao,
            // 'cuahang_id' => $this->cuahang_id,
            'so_phieu' => $this->so_phieu,
            'created_at' => $this->created_at,
            'updated_at' => $this->updated_at,
            'user_add' => $this->user_add,
        ]);
        $query->andFilterWhere(['IN', 'cuahang_id', json_decode(getUser()->cuahang_id)]);

        $query->andFilterWhere(['like', 'note', $this->note])
            ->andFilterWhere(['like', 'status', $this->status])
            ->andFilterWhere(['like', 'cuahang.name', $this->status]);

        return $dataProvider;
    }
}
