...
// overflow
            trigger_error(
                'docsotien only accepts numbers between -' . PHP_INT_MAX . ' and ' . PHP_INT_MAX,
                E_USER_WARNING
            );
            return false;
        }
        if ($number < 0) {
            return $hh4 . self::docsotien(abs($number));
        }
        $string = $fraction = null;
        if (strpos($number, '.') !== false) {
            list($number, $fraction) = explode('.', $number);
        ...
        switch (true) {
            ...
            if ($remainder) {
                $string .= $hh2 . self::docsotien($remainder);
            }
            break;
            default:
            $baseUnit = pow(1000, floor(log($number, 1000)));
            $numBaseUnits = (int) ($number / $baseUnit);
            $remainder = $number % $baseUnit;
            $string = self::docsotien($numBaseUnits) . ' ' . $ht[$baseUnit];
            if ($remainder) {
                $string .= $remainder < 100 ? $hh2 : $hh3;
                $string .= self::docsotien($remainder);
            }
            break;
        }
        if (null !== $fraction && is_numeric($fraction)) {
