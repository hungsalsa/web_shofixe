...
<div class="sanpham-thongke-index chuthich">
    ...
    <h1 class="col-md-6"><?= Html::encode($this->title) ?></h1><h3 class="pull-right col-md-6">Hãy <?= Html::a('cập nhật','/backend/common/sanpham-thongke/capnhatton',['class'=>'btn btn-success p-4']) ?> số lượng tồn trước khi thống kê</h3>
 ...
 <?php foreach ($search['cuahang_query'] as $keyCH => $cuahang): ?>
 	...
 	<div class="panel panel-default">
 						...
 						<table class="table full-color-table full-success-table hover-table thonhke">
 							<tbody>
 								<?php $i = 1;   foreach ($data as $keysp => $value): ?>
 									<?php 
 									// dbg($search['category_query']);
 									// echo 'dung ko ?' .(in_array($value['masp'],$dataProductquery));
 									if (in_array($value['masp'],$dataProductquery)) {
 										
 									}
 									// pr($value['masp']);
 									// dbg($dataProductquery);
	 									if ($value['cuahang_id'] != $keyCH || $value['cate_id'] != $keyCate ){continue;}
	 									// if ($value['cate_id'] != $keyCate){continue;} 
 									// dbg($dataProductquery);
 									...
 									<tr>
 										...
 										<td><?= Yii::$app->formatter->asInteger($value['tiennhap'])?></td>


 										<td><?= $value['slxuat'] + $value['slxuatnb'] ?></td>
 										<td><?= Yii::$app->formatter->asInteger($value['tienxuat'])?></td>
	 	...
	 	?>
	 	<div class="panel-heading text-center bg-warning"><?= $category ?> <span style="margin-left: 30px"></span>Tổng số lượng: Tổng số lượng : <?= ($countPro==0)? 0 :$countPro ?>
		 	<div class="panel-action"><a href="#" data-perform="panel-collapse"><i class="ti-minus"></i></a> <a href="#" data-perform="panel-dismiss"><i class="ti-close"></i></a></div>
		 </div>
 		 <?php endif ?>
 	</div>	
