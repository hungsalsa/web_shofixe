    ...
    $this->title = 'video '.$data['category']['CateTitle'];
    $time = new Mydatetime();
    ?>
        ...
        <section class="main_section_home">
            <div id="top_inner" class="box_cat">
                <div class="title_box">
                    <h1>Các video <?= $data['category']['cateName'] ?></h1>
                </div>
                <?php foreach ($data['video'] as $key => $value): 
                // if ($data['id'] == $value['idVideo']) {
                //     continue;
                // }
                // dbg(count($data['video']));
                    ?>
                    <?php if ($key==0)://dbg($value); ?>
