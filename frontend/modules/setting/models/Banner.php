...
class Banner extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['content', 'content_mobile', 'status', 'created_at', 'updated_at', 'user_add'], 'required'],
    ...
    public function getBanner($status = true)
    {
        return self::find()->select(['content','content_mobile'])->where('status=:active',[':active'=>$status])->one();
    }
}
